...
def get_clang_flags(*cmd):
  cache_value = em2clang.get(cmd)
  if cache_value is None:
    cache_value = shared.check_call(cmd + ('--cflags',), stdout=shared.PIPE).stdout
    em2clang[cmd] = cache_value
  return cache_value
...
rule direct_cc
  ...
  command = $CC -MD -MF $with_depfile $CFLAGS -c $in -o $out
  description = CC $out
    ...
    for src in input_files:
      ...
      if customize_build_flags:
        ...
        if custom_flags != flags:
          out += f'  {flagsname} = {get_clang_flags(shared.EMCC if cmd != "cxx" else shared.EMXX, *custom_flags)}\n'
      out += '\n'
