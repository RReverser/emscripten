...
def create_ninja_file(input_files, filename, libname, cflags, asflags=None, customize_build_flags=None):
  ...
  def wrap_build_dir_cmd(cmd):
    cmd = 'cd $build_dir && ' + cmd
    return f'cmd /c {cmd}' if sys.platform == 'win32' else cmd
...
EMXX = {shared.EMXX}
EMAR = {shared.EMAR}
build_dir = {escape_ninja_path(build_dir)}
  ...
  if suffix == '.o':
    ...
    for (cmd, custom_flags), files in batches.items():
      while files:
        ...
        chunk_o = [os.path.join(build_dir, shared.unsuffixed_basename(src) + '.o') for src in chunk]
        objects += chunk_o
        shared_name = f'{chunk_o[0]}'
        out += f'build {" ".join(escape_ninja_path(o) for o in chunk_o)}: {cmd} {" ".join(escape_ninja_path(src) for src in chunk)}\n'
        out += f'  shared_name = {escape_ninja_path(shared_name)}\n'
        if custom_flags:
