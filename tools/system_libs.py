...
def create_ninja_file(input_files, filename, libname, cflags, asflags=None, customize_build_flags=None):
  ...
  def get_clang_flags(cmd, flags):
    cache_key = (cmd, *flags)
    cache_value = em2clang.get(cache_key)
    if cache_value is None:
      cache_value = shared.check_call([cmd, *flags, '--cflags'], stdout=shared.PIPE).stdout
      em2clang[cache_key] = cache_value
    return cache_value
